...
Useful local env vars: `GITHUB_TOKEN` (a GitHub personal access token — classic PAT with no scopes is enough for public read; raises REST from 60→5000 req/hr and Search from 10→30 req/min), `UPSTASH_REDIS_URL` (cache no-ops without it — Redis also makes repeated search queries near-instant), `CACHE_TTL_SECONDS`, `CACHE_REFRESH_SECRET` (required value for `?refresh=` cache bust on `/api/release/...` and for `GET|POST /api/archive/sync`; ignored when unset), `BLOB_READ_WRITE_TOKEN` (Vercel Blob — required to sync archive mirrors), `ARCHIVE_GROUPS` (semicolon-separated alias groups, e.g. `permissionlesstech/bitchat|argval/bitchat;permissionlesstech/bitchat-android|argval/bitchat-android`; empty disables archive), `CRON_SECRET` (optional alternate Bearer for Vercel Cron hitting `/api/archive/sync`), `RATE_LIMIT_RPM`, `SEARCH_RATE_LIMIT_RPM` (stricter per-IP cap on `/api/search`, default 20), `PORT`.
...
- **`archive/`** — optional permanent mirrors for allowlisted repo groups (`ARCHIVE_GROUPS`). Sync (`GET|POST /api/archive/sync`, gated by `CACHE_REFRESH_SECRET` / `CRON_SECRET`) pulls each group's latest release (preferred alias first, then forks) plus a source zip into Vercel Blob (`BLOB_READ_WRITE_TOKEN`) and writes durable Redis manifests (no 24h hard TTL). `/dl`, `/api/link`, and `/api/release` overlay Blob URLs for those aliases and fall back to the last synced manifest when GitHub is unreachable. Daily Vercel Cron hits `/api/archive/sync`.
- **`handlers/`** — Gin route handlers, one file per route family (`redirect`, `link`, `page`, `releases`, `search`, `archive`). Each handler fetches via `cache.FetchCached(ctx, cache, key, ghClientCall)`, then hands assets to `picker` if it needs to pick one.
